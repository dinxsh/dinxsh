### Indie hacker building on-chain products for 1M+ users
![](https://komarev.com/ghpvc/?username=dinxsh) [![wakatime](https://wakatime.com/badge/user/018cddd8-b17b-4e5f-a792-bed4da250ea7.svg)](https://wakatime.com/@018cddd8-b17b-4e5f-a792-bed4da250ea7)

### work
- [infra to solve the plastic crisis](https://fluxava.xyz/)
- [dev for starknets first evidence-based onchain analytic systems](https://www.prismbyaegis.com/)
- [anon social platform with private prediction markets powered by arcium](http://anonn.space/)

### projects
- [polymarket arbitrage agent with risk limits](https://github.com/dinxsh/PolyShark)
- [ethereum → stacks USDC bridge for seamless transfers](https://github.com/dinxsh/usdcx-hub)
- [solana → zcash shielded bridge for SOL/USDC to ZEC with relayers](https://github.com/dinxsh/zwap)
- [merkle-verified file storage with cryptographic proof for downloads](https://github.com/dinxsh/MerkSeal)
- [AI ad agents using x402 micropayments for autonomous ad placement](https://github.com/dinxsh/adkit)


<details>
<summary>recent activity</summary>

  
| Overview |
|:--------:|
| ![Lines of Code & Base Introduction](assets/metrics.plugin.code.lines.svg) |


</details>

<sub>most active on [Discord](https://t.co/QPthpsZ1Qu) or [X/Twitter](https://x.com/dineshcodes)</sub>
